...
class LongName(EFLR):
    ...
    def __init__(self, object_name: str, set_name: str = None, **kwargs):
        ...
        self.quantity_modifier = self._create_attribute('quantity_modifier', converter=self.convert_values)
        self.altered_form = self._create_attribute('altered_form')
        self.entity = self._create_attribute('entity')
        self.entity_modifier = self._create_attribute('entity_modifier', converter=self.convert_values)
        ...
        self.source_part = self._create_attribute('source_part', converter=self.convert_values)
        self.source_part_number = self._create_attribute('source_part_number', converter=self.convert_values)
        self.conditions = self._create_attribute('conditions', converter=self.convert_values)
