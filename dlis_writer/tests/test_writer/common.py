...
from dlis_writer.utils.source_data_objects import NumpyInterface, HDF5Interface
...
def reference_data(reference_data_path):
    return load_hdf5(reference_data_path)
...
def short_reference_data(short_reference_data_path):
    return load_hdf5(short_reference_data_path)
...
def write_dlis_file(data, dlis_file_name, config):
    ...
    if isinstance(data, np.ndarray):
        ...
        raise TypeError(f"Expected a numpy.ndarray or a path to a HDF5 file; got {type(data)}")
